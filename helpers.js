...
const validateObject = (obj, objName) => {
    ...
    if (Object.values(obj).length === 0)
        throw `Error: ${objName || 'Provided parameter'
...
const checkArrLength = (arr, arrName, min) => {
    if (min !== undefined && min !== null) {
        if (arr.length < min)
            throw `Error: ${arrName || 'Provided parameter'
...
const checkMinValue = (num, numName, min) => {
    if (num < min)
        throw `Error: Minimum value of ${numName || 'Provided parameter'
        } should be ${min}`;
    return num;
};
...
const checkMaxValue = (num, numName, max) => {
    if (num > max)
        throw `Error: Maximum value of ${numName || 'Provided parameter'
        } should be ${max}`;
    return num;
};
