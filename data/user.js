...
export const updateUser = async (userId, updateObject) => {
  ...
  let updatedUser = await userCollection.updateOne({_id: new ObjectId(userId)}, {$set: newObj});
...
export const loginUser = async (email, password) => {
  ...
  if(check) {
    return {id: user._id.toString(), firstName: user.firstName, middleName: user.middleName, lastName: user.lastName, email: user.email, gender: user.gender, age: user.age, city: user.city, state: user.state, profilePicture: user.profilePicture};
  }
  else {
...
export const getFavorites = async (userId) => {
  ...
  let propCollection = await properties();
  favorites = await propCollection.find({_id : {$in: favouriteIds}}).toArray();
  return favorites;
}
...
export const removeFav = async (userId, propId) => {
  ...
  let userCollection = await users();
  let removed = await userCollection.updateOne({_id: new ObjectId(userId)}, {$pull: {favouriteIds: propId}});
  if (!removed) {
